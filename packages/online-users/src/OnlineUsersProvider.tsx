...
import { type ComponentType, type FC, type ReactElement, type ReactNode, useEffect, useRef, useState } from "react";
		...
		return (
			<OnlineUsersProvider
				...
				user={{
					...
					name: window.application.sandbox.config.context?.user?.name,
					// @ts-expect-error the hook makes a fetch request :(
					avatar: window.application.sandbox.config.context?.user?.preview_url_without_placeholder,
				}}
			>
				<Component {...props} />
			</OnlineUsersProvider>
