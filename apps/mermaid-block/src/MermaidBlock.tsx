...
import { type FormEventHandler, type ReactElement, useCallback, useEffect, useRef, useState } from "react";
...
export const MermaidBlock = ({ appBridge }: BlockProps): ReactElement => {
	...
	const handleHeightChange = (height: number) => {
		setBlockSettings({ height: `${height}px` }).catch(() => console.error("Couldn't save the block settings"));
	...
	const renderMermaid = useCallback(async () => {
		mermaidImport.current ??= await import(/* @vite-ignore */ `https://esm.sh/mermaid@${packageJson.dependencies.mermaid}`).then(
			(mod) => mod.default,
		);
	...
	return (
		<div
			data-test-id="mermaid-block"
			className={cn(styles.root, "sa-root", "mermaid-block")}
			style={getMermaidRootStyle(blockSettings)}
		>
			{isEditing ? (
				<>
					<ResizeWrapper>
						<div className={styles.relativeWrapper}>
							<pre
								...
								className={cn(styles.output, borderClasses, loading && styles.loading)}
							/>
					...
					<form className={styles.form} onSubmit={handleMermaidSave}>
						<h3>Mermaid Code</h3>
						<textarea
							name="mermaid-code"
				...
				<div className={styles.display} data-show-code={blockSettings.alwaysShowCode || showCode}>
					<div style={{ height: blockSettings.height }} className={cn(styles.container, borderClasses)}>
						<div
						...
						{!blockSettings.alwaysShowCode && blockSettings.displayShowCodeButton && (
							<div className={styles.showCodeButton}>
